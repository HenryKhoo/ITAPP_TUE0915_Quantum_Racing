...
# Corkscrew Edition — tuned from lap telemetry torcs_log_20260421_170839
#############################################
...
# --- NEW ---
_WALL_RECOVERY_SPEED    = 32   # km/h below which "steer-first" mode engages
MAX_BRAKE       = 0.72   # ceiling for ramped zones
_RAMP_DIVISOR   = 90     # higher = gentler ramp


# --- NEW ZONE: early hard-braking for high-speed corner entry ---
_HIGHSPEED_ENTRY_SPEED   = 140   # km/h — above this, use hard entry braking
_HIGHSPEED_ENTRY_AHEAD   = 52 
_HIGHSPEED_ENTRY_TARGET  = 90    # km/h — speed we want AT the apex
_HIGHSPEED_ENTRY_CEILING = 0.82  # max brake force for this zone

# Add these constants near the other zone parameters:
_CORKSCREW_BRAKE_START = 2375.0
_CORKSCREW_BRAKE_END   = 2405.0   # finishes 5 m before crest — straight line complete
_CORKSCREW_ENTRY_TARGET = 72.0   # slight raise — car has less time to scrub, don't overshoot
_CORKSCREW_CREST_START = 2410.0
_CORKSCREW_CREST_END   = 2445.0
_CORKSCREW_CREST_SPEED = 70.0   # NEW — allows natural speed through the descent

# --- FINAL CORNER HARD BRAKE ZONE ---
_FINAL_CORNER_BRAKE_START = 3220.0   # CHANGED: was 3230 — arm 10m earlier, catches highspeed_entry handoff
_FINAL_CORNER_BRAKE_END   = 3310.0   # was changed to 3290.0 — revert
_FINAL_CORNER_TARGET_SPEED = 60.0    # was changed to 75.0 — revert
_FINAL_STRAIGHT_START = 3315.0       # was changed to 3295.0 — revert
_FINAL_STRAIGHT_END   = 3608.0       # unchanged

# --- CHANGE: new medium-speed corner zone ---
_MEDIUM_ENTRY_SPEED  = 85   # km/h — arm below this speed threshold  (was: no zone)
_MEDIUM_ENTRY_TARGET = 58   # km/h — target speed through tight corners (was: no zone)
_MEDIUM_ENTRY_AHEAD  = 55   # m    — ahead sensor to arm it            (was: no zone)
...
def wall_filtered_lookahead(S):
    """
    Suppresses the Corkscrew false wall reading only.
    The Corkscrew false positive appears specifically at dist ~2430-2460,
    speed 40-80 km/h, nearly centred, low angle.
    Tightened: suppress only when speed < 85 km/h (not 120).
    This lets the emergency_wall fire normally on all real corners.
    """
    ...
    if angle > 0.12:
        return ahead
    if speed > 85:           # CHANGED: was 120 — restores emergency_wall on real corners
        return ahead
    if ahead < 45:
        return 200           # suppress Corkscrew slope artifact only
...
def calculate_steering(S):
    ...
    # --- NEW: corner pre-rotation ---
    # When a corner is approaching (ahead shrinking, car still straight),
    # blend in a small bias steer toward the corner direction so the car
    # arrives already yawed into the bend rather than square to it.
    ...
    if corner_approaching:
        # Determine corner direction from side sensors: shorter side = inside wall
        # right wall closer → left-hand corner → need rightward pre-rotation (positive steer)
        side_diff = left - right   # positive = left wall further = right-hand bend
        # Scale: max bias 0.18 steer, fades as corner tightens (ahead shrinks)
    ...
    # --- end pre-rotation ---
     ...
     # --- FINAL CORNER EXIT: reduce centering gain to stop steer hunt ---
    dist = S.get('distFromStart', 0)
    _in_final_exit = 3275.0 <= dist <= 3410.0
    # --- end zone flag ---
    
     # --- HIGH-SPEED CENTERING DEADBAND (NEW) ---
    # At speeds above 140 km/h, centering on a nearly-centred car causes
    # frame-to-frame sign flips between angle_steer and centering,
    # producing left-right steer oscillation (~±0.021 every frame).
    # Suppress centering entirely when the car is well within the lane
    # and travelling fast. This removes the wobble with zero handling penalty.
    if speed > 140 and abs(track_offset) < 0.30:
        # Only angle-based correction — no centering interference
    ...
    # --- FINAL CORNER ENTRY: force car to left side of track for apex ---
    # Car enters at trackPos ~+0.14 (outside). Need it at -0.10 to -0.20
    # so the exit arc has room. Active only on the braking/entry phase.
    _in_final_entry = 3215.0 <= dist <= 3262.0   # arm 5m earlier, hold 4m longer
    if _in_final_entry:
        position_error = S['trackPos'] - (-0.30)  # was -0.15 — pull car further left before apex
        entry_correction = position_error * 0.65  # was 0.45 — stronger pull
...
def calculate_throttle(S, steer):
    ...
    if S['speedX'] < speed_target * 0.93:   # was 0.90 — wider full-throttle window
        accel = 1.0
    elif S['speedX'] < speed_target:
        accel = 0.82                          # was 0.78 — more aggressive recovery
    else:
...
def apply_brakes(S, steer):
   ...
   # --- FINAL CORNER FORCED DECELERATION ---
    ...
    # *** NEW: CORKSCREW PRE-CREST FORCED DECELERATION ***
    # _CORKSCREW_BRAKE_START / _CORKSCREW_BRAKE_END were defined but never
    # wired into apply_brakes(). At dist 2360 the car is ~103 km/h and
    # re-accelerates to 122 by 2400. This block enforces the entry target.
    if _CORKSCREW_BRAKE_START <= dist <= _CORKSCREW_BRAKE_END:
        excess = max(S['speedX'] - _CORKSCREW_ENTRY_TARGET, 0)
        if excess > 0:
            force = min(excess / 48.0, 0.78)   # was / 45.0 — tighter ramp, hits target harder
    ...
    if ahead_filtered < 20 and S['speedX'] > 55:
        # CHANGED: use angle as proxy for "already correcting" — steer may be
        # suppressed by the corkscrew crest block even when the car is yawing.
        # Also guard: if we're still inside the crest zone, never fire emergency_wall.
    ...
    # --- HARD GATE: no braking at all through the Corkscrew crest ---
    # The slope geometry makes ahead sensors read ~20m (false wall).
    # Any brake here destabilises the car mid-drop. Trust the crest block.
    if _CORKSCREW_CREST_START <= dist <= _CORKSCREW_CREST_END:
        return 0.0, 'corkscrew_crest_gate'
    ...
    if ahead_filtered < 65 and S['speedX'] > 125:   # was 105 — stops over-braking on S5 approach curves
        return compute_brake_force(115), 'pre_corner_near'  # also raise target from 105→115


    if very_near_edge and S['speedX'] > 30:
        if _FINAL_STRAIGHT_START <= dist <= _FINAL_STRAIGHT_END:
            return 0.0, ''   # NEW: no edge braking on the straight — let car accelerate out
        if abs(steer) < 0.55:
...
def drive_modular(c, logger=None):
    ...
    # --- CORKSCREW CREST: suppress ALL braking, hold steady throttle ---
    if is_corkscrew_crest(S):
        ...
        if speed_now > _CORKSCREW_CREST_SPEED:
         ...
         # --- CHANGE: position-aware crest steering (was: fixed 0.22) ---
        track_pos = S.get('trackPos', 0)
        # Target trackPos = 0.0 (centre). Pull toward it gently during crest.
        # Cap correction so we don't fight the physics of the drop.
        centering_correction = track_pos * 0.55   # positive trackPos = right of centre → steer left
        raw_crest_steer = S['angle'] * 8.0 / math.pi - centering_correction
        raw_crest_steer = max(-0.30, min(0.30, raw_crest_steer))   # hard cap: stay stable
        R['steer'] = raw_crest_steer
        # --- END CHANGE ---
        #  
        R['gear'] = shift_gears(S)
        # *** FIX: early return so calculate_throttle/apply_brakes can't overwrite ***
        if logger:
